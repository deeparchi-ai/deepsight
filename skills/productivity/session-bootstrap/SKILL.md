...
> ⚠️ 仅限无 profile 的 default agent 使用 `~`。Profile agent 需改用绝对路径 `/home/kuang/.hermes/`。
...
3. **Git commit**：`~/.hermes` 已初始化为 git repo。涉及以下文件变更时，使用 `cd ~/.hermes && git add <files> && git commit -m "<message>"` 提交：
   - `skills/` 目录下的任何修改
   - `task_registry.json`
   - `progress.log`
   - `SOUL.md`（persona 变更）
   ...
   ```
   cd ~/.hermes
   git config user.email "kuangmi@deeparchi.com"
   git config user.name "邝谧"
   ```
...
- `references/bridge-integration.md` — 外部桥接工具（如 feishu-cc-bridge）的 Bootstrap 集成模式，含 Node.js 实现参考和跨 OS 访问方案

## 外部桥接集成

Bootstrap 的状态文件（`task_registry.json` + `progress.log`）可被非 Hermes 工具读取。典型场景：feishu-cc-bridge（Windows Node.js → 飞书 → Claude Code）通过 `wsl cat/tail` 从 WSL 读取 Hermes 状态，在飞书 DM 中回复 `/bootstrap` 命令。详见 `references/bridge-integration.md`。
