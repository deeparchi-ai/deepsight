...
**2.0 模型多样性预检：** ≥3 不同厂商 → 正常执行；1-2 → 降级执行；0 → 跳过（增强对抗审查替代）。

**2.1 并行盲审：** `delegate_task` batch 模式，五角色并行审计（唱反调者 / 第一性原理 / 扩张主义者 / 局外人 / 执行者）。每个角色只接收原始问题 + 结论摘要，不接收推理链。

**2.2 互评 + 主席综合：** 主模型逐条回应审计意见 → 识别共识（≥3位独立提出） → 裁决分歧 → 修正结论和证据等级。

输出：`审计报告`（共识批评 + 分歧裁决 + 证据等级修正表 + 总体意见）。

配置参考：`references/council-model-setup.md`。
